...
class TreeSeg:
    ...
    def get_embeddings_local(self, chunks):
        # returns list[np.ndarray], one per chunk
        embs = self.embedder.encode(
            chunks,
            batch_size=self.hf_batch_size,
            convert_to_numpy=True,
            normalize_embeddings=self.hf_normalize,
            show_progress_bar=False,
        )
        # ensure python lists of floats (to match downstream usage that expects JSON-like lists)
        return [emb.tolist() for emb in embs]

    def embed_blocks(self):
        logger.info(f"Embedding blocks locally with Hugging Face: {self.configs['HF_EMBEDDING_MODEL']}")
        
        N = len(self.blocks)
        BATCH_SIZE = self.hf_batch_size  # Use the HF batch size consistently
        ...
        embs = []
        result = [
            self.get_embeddings_local([block["convo"] for block in batch])
